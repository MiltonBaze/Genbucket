...
def rodar_analises(bucket_url, relatorio_dir, config):
    url = f"https://{bucket_url}" if not bucket_url.startswith("http") else bucket_url
    print(f"\n🔍 Analisando: {url}")
    resultado = f"Bucket: {bucket_url}\n"

    wapiti_path = config.get("wapiti_path", "tools/wapiti.exe")
    nuclei_path = config.get("nuclei_path", "tools/nuclei.exe")
    ...
    try:
        ...
        executar_wapiti(url, saida_html, wapiti_path)
    ...
    try:
        ...
        executar_nuclei(url, relatorio_dir, nuclei_path)
        ...
        if versao.lower().startswith("v_"):
            ...
            if os.path.exists(resultado_path):
                ...
                for bucket_url, provedor in buckets:
                    resumo = rodar_analises(bucket_url, relatorio_dir, config)
