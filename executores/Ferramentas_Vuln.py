...
def executar_wapiti(url, saida_html, wapiti_path, timeout=300):
    wapiti_path = os.path.abspath(wapiti_path)
    os.makedirs(os.path.dirname(saida_html), exist_ok=True)
    print(f"  [WAPITI] → Executando: {wapiti_path}")
...
def executar_nuclei(url, relatorio_dir, nuclei_path, timeout=300):
    ...
    print(f"  [NUCLEI] → Executando: {nuclei_path}")
